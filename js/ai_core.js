...
window.envContext = "SYSTEM AWARENESS: The user's exact location is still being acquired.";
if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(async pos => {
        try {
            let lat = pos.coords.latitude;
            let lon = pos.coords.longitude;
            let [mRes, geoRes] = await Promise.all([
                fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current_weather=true`),
                fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}`)
            ]);
            let mData = await mRes.json();
            let geoData = await geoRes.json();
            let temp = mData.current_weather.temperature;
            let loc = geoData.address.village || geoData.address.suburb || geoData.address.neighbourhood || geoData.address.town || geoData.address.city || "their precise local area";
            window.envContext = `SYSTEM AWARENESS: The user is currently physically located in ${loc}. The exact real-time live local weather there is ${temp}°C.`;
        } catch(e) {}
